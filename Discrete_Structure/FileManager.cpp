...
vector<User> FileManager::getUserByRole(string role) {
	vector<User> users = loadUsers();
	if (users.empty()) {
		cout << "No User found. \n";
		return users;
	}
	vector<User> roleUsers;
	
	for (int i = 0; i < users.size(); i++) {
		if (users[i].getRole() == role) {
			roleUsers.push_back(users[i]);
		}
	}
	cout << roleUsers.size() << " Users of this role has found\n";

	return roleUsers;  

}

vector<Door> FileManager::getDoorByAccessLevel(int level) {
	vector<Door> doors = loadDoors();
	vector<Door> result;
	if (doors.empty()) {
		cout << "No User found. \n";
	...
	for (int i = 0; i < doors.size(); i++) {
		if (doors[i].getAccessLevel() == level) {
			result.push_back(doors[i]);
		}
	}

	cout << result.size() << " number of doors of this accessLevel has found\n";
	return result;

}

vector<Door> FileManager::getDoorByType(string type) {
	vector<Door> doors = loadDoors();
	vector<Door> result;
	if (doors.empty()) {
		cout << "No User found. \n";
	...
	for (int i = 0; i < doors.size(); i++) {
		if (doors[i].getDoorType() == type) {
			result.push_back(doors[i]);
		}
	}

	cout << result.size() << " number of doors of this type has found\n";
	return result;
